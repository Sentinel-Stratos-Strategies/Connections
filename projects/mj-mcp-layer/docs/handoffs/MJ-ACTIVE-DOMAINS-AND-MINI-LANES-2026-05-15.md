...
- Route split matches production: **`mj-edge`** → `mj.ellis-aegis.us/*`, `mcp.ellis-aegis.us/*`; **`ellis-aegis-control-plane`** → `api.ellis-aegis.us/*`, `dashboard.ellis-aegis.us/*`, `codex.ellis-aegis.us/*`.
- **`https://api.ellis-aegis.us/api/health`** returned **`200`** with `"status":"ok"` and MUA-related binding flags **true** in the JSON body (control-plane Worker).
- **`mcp.ellis-aegis.us/healthz`**, **`mj.ellis-aegis.us/healthz`**: **`200`** (`mj-edge`).
- **Zones `hitch.guru` / `kevis.online`:** still **no usable public apex** from DNS as of this audit; see tables below.
...
| `hitch.guru` | active | Apex + `www` proxied **`A` / `CNAME`** added **2026-05-15**; Worker routes **`mcp`**, **`mcp/*`**, **`turn/*`**, **`audit/*`** → **`mj-edge`. Public resolution propagates per resolver. |
| `kevis.online` | active | Apex + `www` proxied **`A` / `CNAME`** added **2026-05-15**; existing **`kevis.online/api/*`** → **`kevis-mj-edge`**; **`mcp`**, **`mcp/*`**, **`turn/*`**, **`audit/*`** → **`mj-edge`. |
...
| `api.ellis-aegis.us` | active: `/` `200`, `/healthz` `200`, **`/api/health` `200`**, `/mcp` protected `401` without token | `ellis-aegis-control-plane` | API/control-plane host (`/api/health` is the primary JSON health for this worker). |
...
| `hitch.guru` | not verified from this CI host | `mj-edge` (see routes) | **DNS + routes installed 2026-05-15** — confirm with `curl` / browser after propagation. |
| `kevis.online` | not verified from this CI host | `kevis-mj-edge` + `mj-edge` (see routes) | **DNS + routes installed 2026-05-15** — confirm `dig` / HTTPS after propagation. |
...
| `hitch.guru` | `hitch.guru/mcp` | `mj-edge` |
| `hitch.guru` | `hitch.guru/mcp/*` | `mj-edge` |
| `hitch.guru` | `hitch.guru/turn/*` | `mj-edge` |
| `hitch.guru` | `hitch.guru/audit/*` | `mj-edge` |
| `kevis.online` | `kevis.online/mcp` | `mj-edge` |
| `kevis.online` | `kevis.online/mcp/*` | `mj-edge` |
| `kevis.online` | `kevis.online/turn/*` | `mj-edge` |
| `kevis.online` | `kevis.online/audit/*` | `mj-edge` |
...
**Repository:** `projects/mj-mcp/lanes/*/lane.manifest.json` — **36** lane packages checked in (mini-lane definitions + governance).  
**Identity map:** `projects/mj-mcp/lanes/lane-identities.v1.json` (mirrored in Cloudflare-ellis-aegis) lists the public host/path for each lane.  
**Live operator verification:** `GET https://api.ellis-aegis.us/api/console/lanes` — after **`mj-perplexity`** lands in **`ellis-aegis-control-plane`**, expect **`count`: 34** and **3** skipped lanes (`mj-computer`, `mj-gadget`, `mj-railway`), matching the Worker registry + static skipped list (re-verify post-deploy).
...
| `mj-antigravity` | Antigravity | `remote_mcp_operator` | `https://mcp.ellis-aegis.us/mcp` | `mcp.admin`, `script.run`, `forensic.read` | `ready_for_operator_token` |
...
| `mj-perplexity` | Perplexity | `api_or_connector` | `https://mcp.ellis-aegis.us/mcp` | `doc.read`, `forensic.read`, `tool.call_approved` | `ready_for_api_token` |
...
| `mj-orbstack` | OrbStack | `local_bridge` | `http://127.0.0.1:8789/mcp` | `mcp.admin`, `forensic.read` | `ready_for_local_bridge` |
